...
func approveKey(c *runCtx, key string) error {
	...
	if _, err := os.Stat(cur); err != nil {
		return fmt.Errorf("no current capture for %q — run `gdviz capture` first", key)
	}
...
func pendingKeys(c *runCtx) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("no report at %s — run `gdviz test` first", filepath.Join(c.output, "report.json"))
	}
	var keys []string
...
func cmdApprove(argv []string) error {
	...
	if len(keys) == 0 {
		fmt.Println("gdviz: nothing to approve")
		return nil
	}
	...
	fmt.Printf("gdviz: %d baseline(s) updated\n", len(approved))
	return nil
}
