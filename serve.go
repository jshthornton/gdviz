...
func initTemplates() error {
	...
	t, err := template.New("gdviz").Funcs(funcs).ParseFS(templateFS, "web/templates/*.html")
	if err != nil {
		return err
	}
	reviewTmpl = t
...
func (s *server) handleShotFragment(w http.ResponseWriter, key string) {
	...
	if d == nil {
		...
		w.Write([]byte(`<div class="empty">shot ` + template.HTMLEscapeString(key) + ` is not in the current report — run <code>gdviz test</code></div>`))
		return
	}
...
func cmdReview(argv []string) error {
	...
	fmt.Printf("gdviz review: %s  (ctrl-c to stop)\n", url)
	if !*noOpen {
		go func() {
