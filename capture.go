...
func loadCtx(projectFlag string) (*runCtx, error) {
	...
	cfg, err := LoadConfig(filepath.Join(project, "gdviz.toml"))
	if err != nil {
		return nil, err
	}
...
// ensureGodot resolves the Godot binary (flag > $GDVIZ_GODOT > config > PATH)
...
func (c *runCtx) ensureGodot(flagVal string) error {
	...
	return fmt.Errorf("godot binary not found (use --godot, $GDVIZ_GODOT, or godot= in gdviz.toml)")
}
...
func godotArgs(c *runCtx, job Job, record bool, framesDir, overlay string, slot int) []string {
	...
	args = append(args,
		...
		"--position", fmt.Sprintf("%d,%d", 24+slot*48, 24+slot*48),
		"--single-window",
	)
