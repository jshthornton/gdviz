...
//
//	gdviz run [--project DIR] [--width N] [--height N] [--record]
//	          [--set KEY=VAL]... [--godot PATH] SCENE [-- USER_ARGS...]
func cmdRun(argv []string) error {
	fs := flag.NewFlagSet("run", flag.ExitOnError)
	project := fs.String("project", ".", "Godot project root containing gdviz.toml")
	godot := fs.String("godot", "", "godot binary path (else $GDVIZ_GODOT, config, or PATH)")
	width := fs.Int("width", 0, "window width (default: render.width in gdviz.toml, else 1280)")
	height := fs.Int("height", 0, "window height (default: render.height in gdviz.toml, else 720)")
	record := fs.Bool("record", false, "record the run as movie frames under tmp/gdviz/frames/")
	var set sliceFlags
	...
	rest := fs.Args()
	if len(rest) == 0 {
		return fmt.Errorf("usage: gdviz run [flags] SCENE [-- USER_ARGS...]")
	}
	scene := rest[0]
	userArgs := rest[1:]
	...
	if framesDir != "" {
		...
		fmt.Printf("gdviz run: %d frames -> %s\n", frames, framesDir)
	}
	return nil
}
