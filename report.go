...
func compareShot(c *runCtx, r *ShotResult) {
	...
	if diffImg != nil {
		if err := savePNG(filepath.Join(c.output, "diff", r.Key+".png"), diffImg); err != nil {
			fmt.Fprintf(os.Stderr, "gdviz: warning: save diff %s: %v\n", r.Key, err)
		}
	}
}
...
func exitError(r *Report, failOnNew bool) error {
	...
	if bad > 0 {
		return fmt.Errorf("%d shot(s) need attention — run `gdviz review` to inspect and approve", bad)
	}
