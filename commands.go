...
func runTest(argv []string, mode string, compare bool) error {
	...
	project := fs.String("project", ".", "project root containing gdviz.toml")
	...
	parallel := fs.Int("parallel", 0, "max jobs running concurrently (0 = defaults.parallel in gdviz.toml)")
	fs.Parse(argv)
	if fs.NArg() > 0 {
	...
	fmt.Printf("gdviz %s: %d job(s) · godot %s · workers %d · %s\n", mode, len(jobs), c.godotVer, workers, c.project)
	recordMode := 0
	if *noRecord {
		recordMode = 2
	} else if *record {
...
func cmdCompare(argv []string) error {
	...
	if r, err := loadReport(filepath.Join(c.output, "report.json")); err == nil && len(r.Shots) > 0 {
		...
		if err != nil {
			return fmt.Errorf("no current captures — run `gdviz capture` first")
		}
